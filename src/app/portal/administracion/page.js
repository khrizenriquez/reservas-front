...
export default function AdminPage() {
  ...
  return <section className="workflow-page admin-page"><header className="page-heading"><p className="eyebrow">{t("admin.eyebrow")}</p><h1>{t("admin.title")}</h1><p>{t("admin.description")}</p></header>{error ? <StatusMessage code={error} onRetry={load} /> : null}{status ? <p role="status" className="success-message">{t(status)}</p> : null}
    <section className="data-panel" aria-labelledby="labs-title"><div className="panel-heading-row"><h2 id="labs-title">{t("admin.labs")}</h2>{isAdmin ? <button className="button is-primary" onClick={() => { setEditingLab(null); setLabModal(true); }}>{t("admin.createLab")}</button> : null}</div><ul className="record-list">{labsPagination.pageItems.map((lab) => <li key={lab.id} className="record-card"><div><strong>{lab.name}</strong><p className="entity-id">{t("admin.labId")}: <code>{lab.id}</code></p><span className="status-tag">{lab.status === 1 ? t("common.active") : t("common.inactive")}</span></div>{isAdmin ? <button className="button is-small" onClick={() => { setEditingLab(lab); setLabModal(true); }}>{t("admin.editLab")}</button> : null}</li>)}</ul><PageControls pagination={labsPagination} count={data.labs.length} t={t} /></section>
