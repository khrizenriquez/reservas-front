...
export function createRenderApiClient({ baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || DEFAULT_BASE_URL, fetchImpl = fetch } = {}) {
  ...
  return {
    ...
    listReservations: operation("/api/reservas/", "GET"), createReservation: operation("/api/reservas/", "POST"), getReservation: operation(({ id }) => `/api/reservas/${id}/`, "GET"), updateReservation: operation(({ id }) => `/api/reservas/${id}/modificar/`, "PUT"), cancelReservation: operation(({ id }) => `/api/reservas/${id}/cancelar/`, "PATCH"), listAuditLogs: operation("/api/logs/", "GET")
  };
}
