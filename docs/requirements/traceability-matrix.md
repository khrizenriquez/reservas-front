...
| HU-018-S06 | Render labs, conditions, users and logs operations; CST-001, CST-010 | `src/app/portal/administracion/page.js`, `src/app/portal/logs/page.js` | administration and logs page tests; `AuditVisuals.test.js` | Admin-only mutations, real Render logs, original SVG/CSS charts and modal workflows are covered. Backend authorization remains a security TODO. |
...
keeps the published query editable, and presents local-only audit metrics, original
SVG/CSS visualizations and 10/20/50 pagination. No analytics endpoint or server
pagination is invented. Raw API values are not translated.
