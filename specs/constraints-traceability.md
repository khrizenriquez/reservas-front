...
| CST-009 | Vercel deployment readiness | `vercel.json` runs the release gates and `next.config.mjs` provides the security headers; preview/production connectivity uses the same Render-only anonymous contract. | HU-018-S01..HU-018-S08 | `npm run release:check`, deploy URLs, `cors:check`, and release evidence. |
