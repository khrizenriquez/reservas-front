...
- [x] **22 — `feature/weekly-logs-users`: make audit periods and user administration explicit.** Merged in PR #28. The dedicated Admin-only users route, real-data weekly Logs chart, validated local date range, and laboratory/condition administration are delivered. Backend enforcement of identity and roles remains a security TODO.
- [ ] **23 — `feature/operational-dashboard`: add project-resource indicators and operational IDs.** Show the real Render user ID in the administrator accounts table and each laboratory ID in Administration. Under Logs, administrators receive independently loaded indicators for labs, conditions, reservations, and users; professors retain audit analytics without a user-resource request or panel. The dashboard may use only project data published by Render v1 — never CPU, RAM, uptime, pods, or simulated server telemetry. Local evidence is green on 2026-08-22: committed and live Render contracts, Vercel-origin CORS, lint, 80 Jest tests (92.45% statements / 81.44% branches), release check, production build, and local HTTP smoke for Logs/Users/Administration. Awaiting commit, push, PR, and Vercel Preview.
