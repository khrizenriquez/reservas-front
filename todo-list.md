...
- [ ] **02 — `feature/next-scaffold`: create the executable foundation.** Initialize Next.js App Router in JavaScript, add Bulma, the approved folder boundaries, `.env.example`, linting, Jest, coverage threshold (>80%), and scripts `contract`, `check`, and `test:jest`. Add a minimal rendered-route test and verify the clean build.
- [ ] **03 — `feature/render-contract-gate`: verify the Render v1 contract.** Add a dependency-free contract verifier that checks the committed snapshot and required `/api/*` operations against the Render schema. Include fixtures and unit tests for matching, missing, and drifted operations. It must only validate Render v1.
