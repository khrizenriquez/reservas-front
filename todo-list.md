...
- **Completed:** 16 delivery items; Vercel release evidence and direct anonymous
  Render access are the two remaining PR-scoped items.
...
- [ ] **18 — `fix/anonymous-render-access`: align the client to the anonymous Render v1 contract.** `/portal`, profile, availability, reservations and administration are reachable without login or a client-side role gate; `/acceso` remains an optional login diagnostic. Reservation mutations use only documented fields and never invent a user identity. Rendered/API tests plus README, specs and docs record the backend security TODO. Local `contract`, `check` (50 tests; 95.75 % statements / 81.92 % branches), production build and direct-route HTTP checks passed on 2026-08-16; awaiting PR, preview, merge and branch cleanup.
- [ ] **19 — `feature/experience-i18n`: improve feedback, theme and language coverage.** Add the visible `/portal/logs` route backed only by `GET /api/logs/`; make App Router navigation and in-route Render calls expose localized accessible loading states; use route-level lazy chunks; persist a system-aware light/dark preference; and move every static UI string to the ES/EN configuration. Database data and URLs are intentionally not translated. Local contract, lint, 54 Jest tests (95.88 % statements / 81.37 % branches), release check and production build passed on 2026-08-16; awaiting PR, preview, merge and cleanup.
- [ ] **20 — `feature/client-pagination-modals`: correct audit-log querying and reduce long client lists.** Send the live-required published `UMG_User_ID` query only when entered; add an audit dashboard derived from returned logs; paginate every data list client-side at 10/20/50 items; move long workflows to accessible localized dialogs; and upgrade dark-theme elevation and motion while respecting reduced motion. No API pagination, analytics, authentication, or identity endpoint may be invented. Requires client, component, i18n, documentation, traceability and gate evidence.
